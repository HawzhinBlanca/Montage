...
class AudioNormalizer:
    ...
    def normalize_audio(self, input_file: str, output_file: str,
        ...
        logger.info(f"Starting two-pass audio normalization...")
        ...
        try:
            ...
            logger.info(f"Normalization complete: {stats.input_i:.1f} → {result['output_loudness']:.1f} LUFS")
    ...
    def _analyze_loudness(self, input_file: str) -> LoudnessStats:
        ...
        try:
            ...
            if not json_match:
                raise AudioNormalizationError("Could not parse loudness analysis output")
    ...
    def _apply_normalization(self, input_file: str, output_file: str,
                            stats: LoudnessStats, target: NormalizationTarget) -> None:
        ...
        try:
            ...
            logger.info(f"  Normalization applied successfully")
        except subprocess.CalledProcessError as e:
    ...
    def normalize_segments(self, segments: List[str], output_files: List[str],
                          target: Optional[NormalizationTarget] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Loudness spread: {spread:.1f} LU (min: {min_loudness:.1f}, max: {max_loudness:.1f})")
        ...
        with FFmpegPipeline() as pipeline:
            for i, (segment, output, stats) in enumerate(zip(segments, output_files, all_stats)):
...
def normalize_video_audio(input_video: str, output_video: str,
                          target: Optional[NormalizationTarget] = None) -> Dict[str, Any]:
    """
...
class AudioNormalizedEditor:
    ...
    def process_video_with_normalization(self, input_file: str, output_file: str,
