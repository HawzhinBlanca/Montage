...
class CheckpointManager:
    ...
    def _restore_from_postgres(self, job_id: str, stage: str) -> Optional[Dict[str, Any]]:
        """Restore checkpoint from PostgreSQL if not in Redis"""
        try:
            record = self.db.find_one('job_checkpoint', {
                'job_id': job_id,
                'stage': stage
            })
            ...
            if record:
                ...
                logger.info(f"Restored checkpoint from PostgreSQL for job {job_id} stage '{stage}'")
                return data
    ...
    def get_last_successful_stage(self, job_id: str) -> Optional[str]:
        ...
        try:
            ...
            if not completed_stages:
                # Try PostgreSQL
                records = self.db.find_many('job_checkpoint', 
                    where={'job_id': job_id},
            ...
            completed_stages = {s.decode() if isinstance(s, bytes) else s for s in completed_stages}
