...
import threading
import logging
from metrics import metrics, track_processing_stage, track_api_cost, with_job_tracking
...
class VideoProcessingDemo:
    ...
    def analyze_video(self, job_id: str, video_duration: float):
        ...
        for i in range(num_segments):
            ...
            segments.append({"start": i * 30, "end": (i + 1) * 30, "score": score})
    ...
    def edit_video(self, job_id: str, segments: list):
        ...
        try:
            ...
            metrics.set_ffmpeg_processes(
                max(0, metrics.ffmpeg_processes._value.get() - 1)
            )
    ...
    def print_summary(self):
        ...
        total_cost = sum(
            metrics.cost_usd_total.labels(
                api_name="whisper", job_id=f"demo-job-{i:03d}"
            )._value.get()
            for i in range(100)
        )
        ...
        logger.info(
            f"Success rate: {jobs_completed / (jobs_completed + jobs_failed) * 100:.1f}%"
        )
