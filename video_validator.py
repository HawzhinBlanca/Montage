...
class VideoValidator:
    ...
    def validate_file(self, file_path: str) -> Tuple[bool, VideoMetadata, Optional[str]]:
        """
    ...
    def _run_ffprobe(self, file_path: str) -> Dict[str, Any]:
        ...
        cmd = [
            self.ffprobe_path,
            '-v', 'error',
            '-read_intervals', '%+#2',  # Read first 2 packets to check file integrity
            '-show_format',
            '-show_streams',
            '-of', 'json',
            file_path
        ...
        try:
            ...
            if result.stderr:
                ...
                if 'moov atom not found' in stderr_lower:
                    ...
                    raise CorruptedVideoError("File is corrupted: could not find codec parameters")
            
            if result.returncode != 0:
                raise VideoValidationError(f"ffprobe failed with code {result.returncode}: {result.stderr}")
            ...
            raise VideoValidationError("ffprobe timed out - file may be corrupted or too large")
        except subprocess.SubprocessError as e:
            raise VideoValidationError(f"Failed to run ffprobe: {e}")
    
    def _parse_probe_data(self, probe_data: Dict[str, Any], file_size: int) -> VideoMetadata:
    ...
    def _validate_metadata(self, metadata: VideoMetadata) -> List[str]:
        ...
        if metadata.color_space in hdr_color_spaces or metadata.color_transfer in hdr_transfers:
            errors.append("HDR input not supported")
        
        # Check audio if present
        if metadata.has_audio:
            supported_audio_codecs = ['aac', 'mp3', 'opus', 'vorbis', 'pcm_s16le', 'pcm_s24le']
