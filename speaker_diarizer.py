...
class SpeakerDiarizer:
    ...
    async def diarize_if_needed(self,
                               video_path: str,
                               probe_result: Any = None,
                               segments: Optional[List[Tuple[float, float]]] = None) -> DiarizationResult:
        """
        ...
        if not probe_result:
            ...
            if speaker_estimate <= 1:
                logger.info("Energy analysis suggests single speaker, skipping diarization")
                return self._single_speaker_result()
    ...
    async def diarize_section(self,
                             video_path: str,
                             start: float,
    ...
    async def _estimate_speakers_quick(self, 
                                     video_path: str,
                                     sample_duration: int = 30) -> int:
        """
    ...
    def _detect_speaker_changes(self, 
        ...
        for i in range(1, len(energies)):
            if abs(energies[i] - energies[i-1]) > self.energy_change_threshold:
                ...
                if energies[i] > self.silence_threshold or energies[i-1] > self.silence_threshold:
                    changes += 1
        
        return changes
    
    async def _diarize_segments(self,
                               video_path: str,
                               segments: List[Tuple[float, float]]) -> DiarizationResult:
        """
    ...
    async def _diarize_section_internal(self,
                                       video_path: str,
                                       start: float,
                                       end: float) -> DiarizationResult:
        """
    ...
    async def _diarize_api_section(self,
                                  video_path: str,
                                  start: float,
                                  end: float) -> DiarizationResult:
        """
    ...
    async def _diarize_local_section(self,
                                    video_path: str,
                                    start: float,
                                    end: float) -> DiarizationResult:
        """
    ...
    def _energy_based_diarization(self,
                                 audio_data: np.ndarray,
                                 sample_rate: int,
                                 offset: float) -> List[SpeakerSegment]:
        """
    ...
    async def _find_multi_speaker_sections(self, 
    ...
    async def _extract_audio_section(self,
                                   video_path: str,
                                   start: float,
                                   end: float) -> str:
        """
    ...
    def _merge_speaker_labels(self, 
    ...
    async def _diarize_with_api(self,
                               audio_path: str,
                               start: float,
                               end: float) -> DiarizationResult:
...
async def main():
    ...
    if result.segments:
        ...
        for i, seg in enumerate(result.segments[:5]):  # Show first 5
            print(f"  {seg.speaker_id}: {seg.start_time:.1f}-{seg.end_time:.1f}s "
