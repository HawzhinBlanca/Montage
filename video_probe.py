...
class VideoProbe:
    ...
    def quick_probe(self, video_path: str, sample_seconds: int = 10) -> ProbeResult:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            futures = {
                ...
                executor.submit(self._estimate_speakers, video_path, sample_seconds): 'speakers',
                executor.submit(self._detect_music, video_path, sample_seconds): 'music',
                executor.submit(self._count_scene_changes, video_path, sample_seconds): 'scenes',
                executor.submit(self._analyze_motion, video_path, sample_seconds): 'motion',
                executor.submit(self._detect_faces, video_path, sample_seconds): 'faces'
            }
        ...
        logger.info(f"Probe complete: {probe_result.complexity} complexity, {probe_result.recommended_track} track")
    ...
    def _estimate_speakers(self, video_path: str, sample_seconds: int) -> int:
        ...
        try:
            ...
            energies = [np.sqrt(np.mean(chunk**2)) for chunk in chunks if len(chunk) > 1000]
    ...
    def _detect_music(self, video_path: str, sample_seconds: int) -> bool:
        ...
        try:
            ...
            fundamental_freqs = [82.4, 110, 146.8, 196, 246.9, 329.6, 440, 587.3]  # Musical notes
            harmonic_score = 0
    ...
    def _count_scene_changes(self, video_path: str, sample_seconds: int) -> int:
        ...
        try:
            cmd = [
                ...
                '-vf', 'fps=1,scale=160:90',  # Low res for speed
                '-pix_fmt', 'gray',  # Grayscale for simplicity
            ...
            scene_changes = 0
            prev_frame = cv2.imread(os.path.join(frames_dir, frame_files[0]), cv2.IMREAD_GRAYSCALE)
            
            for frame_file in frame_files[1:]:
                curr_frame = cv2.imread(os.path.join(frames_dir, frame_file), cv2.IMREAD_GRAYSCALE)
    ...
    def _analyze_motion(self, video_path: str, sample_seconds: int) -> Dict[str, float]:
        ...
        if audio_sample:
            try:
                ...
                chunks = [samples[i:i+chunk_size] for i in range(0, len(samples), chunk_size)]
                energies = [np.sqrt(np.mean(chunk**2)) for chunk in chunks if len(chunk) > 100]
    ...
    def _find_peaks(self, signal: np.ndarray, threshold_factor: float = 1.5) -> List[int]:
    ...
    def _recommend_track(self, probe: ProbeResult):
        ...
        logger.info(f"Recommended: {probe.recommended_track} "
