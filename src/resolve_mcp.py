...
class ResolveManager:
    ...
    def add_clips_to_timeline(self, timeline, media_item, clips: List[Dict]) -> bool:
        ...
        try:
            ...
            timeline_items = []
            for clip in clips:
                start_frame = int(clip["start_ms"] * 24 / 1000)  # Convert to frames (24fps)
...
def render_proxy():
    ...
    try:
        ...
        if RESOLVE_AVAILABLE and resolve_manager.connected and resolve_manager.current_project:
...
def status():
    ...
    return {
        ...
        "current_project": resolve_manager.current_project is not None if resolve_manager else False,
        "version": "1.0.0"
    }
...
def start_server(host='localhost', port=7801):
    ...
    print(f"   DaVinci Resolve: {'Available' if RESOLVE_AVAILABLE else 'Not available'}")
    print(f"   Endpoints: /buildTimeline, /renderProxy, /status, /health")
