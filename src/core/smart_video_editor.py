...
from src.core.checkpoint import SmartVideoEditorCheckpoint, CheckpointManager
from src.utils.video_validator import VideoValidator, perform_preflight_check
from src.providers.concat_editor import ConcatEditor, EditSegment, create_edit_segments
from src.providers.audio_normalizer import AudioNormalizer, NormalizationTarget
from color_converter import ColorSpaceConverter
from src.core.metrics import metrics, track_processing_stage, with_job_tracking
...
class SmartVideoEditor:
    ...
    @with_job_tracking
    def process_job(
        self, job_id: str, input_path: str, highlights: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if resume_info:
                ...
                return self._resume_processing(
                    job_id, input_path, highlights, resume_info
                )
    ...
    def _full_processing_pipeline(
        ...
        if not self.checkpoint.should_skip_stage(job_id, "editing"):
            ...
            edit_result = self._edit_video_with_color_safety(
                job_id, input_path, segments, video_duration
            )
        ...
        if not self.checkpoint.should_skip_stage(job_id, "audio_normalization"):
            norm_result = self._normalize_audio(
                job_id, edit_result["temp_output"], video_duration
            )
            self.checkpoint.save_stage_data(
                job_id, "audio_normalization", **norm_result
            )
        else:
        ...
        if not self.checkpoint.should_skip_stage(job_id, "color_verification"):
            ...
            self.checkpoint.save_stage_data(
                job_id, "color_verification", **final_result
            )
        else:
    ...
    def _validate_input(
        self, job_id: str, input_path: str, video_duration: float = None
    ) -> Dict[str, Any]:
    ...
    def _analyze_segments(
        self, job_id: str, highlights: List[Dict[str, Any]], video_duration: float
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_highlight_duration": sum(
                h["end_time"] - h["start_time"] for h in highlights
            ),
        }

    @track_processing_stage("editing")
    def _edit_video_with_color_safety(
        self,
        job_id: str,
        input_path: str,
    ...
    def _normalize_audio(
        self, job_id: str, input_path: str, video_duration: float
    ) -> Dict[str, Any]:
        ...
        norm_result = self.normalizer.normalize_audio(
            input_path, output_path, target=target, video_duration=video_duration
        )
        ...
        return {"output_path": output_path, **norm_result}
    ...
    def _verify_final_output(self, job_id: str, output_path: str) -> Dict[str, Any]:
        ...
        if color_info.color_primaries != "bt709":
            logger.warning(
                f"Output color primaries: {color_info.color_primaries} (expected bt709)"
            )
...
def example_usage():
    ...
    # Define highlights
    highlights = [
        {"start_time": 30, "end_time": 60, "score": 0.9, "transition": "fade"},
        {"start_time": 120, "end_time": 150, "score": 0.85},
        {"start_time": 200, "end_time": 230, "score": 0.88},
    ]

    # Process video
    result = editor.process_job(
        job_id="test-job-001", input_path="/videos/input.mp4", highlights=highlights
    )
