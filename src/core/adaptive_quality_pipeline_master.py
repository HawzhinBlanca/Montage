...
class AdaptiveQualityPipeline:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.monitor = PipelineMonitor(
            daily_budget=config.ai_daily_budget, monthly_budget=config.ai_monthly_budget
        )

        # Setup directories
        self._setup_directories()

        logger.info("🚀 Adaptive Quality Pipeline initialized")
    ...
    async def phase1_ai_processing(self, video_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"✅ Transcript: {len(asr_result.transcript)} chars in {asr_time:.1f}s"
            )
            ...
            local_highlights_path = os.path.join(
                self.config.output_dir, "local_highlights.json"
            )
            ...
            logger.info(
                f"✅ Local scoring: {len(local_highlights)} highlights in {scorer_time:.1f}s"
            )
            ...
            if self.config.use_premium_ai:
                ...
                premium_highlights = await premium_scorer.score_segments(
                    asdict(asr_result), local_highlights
                )

                # Get top highlights based on combined score
                top_premium = sorted(
                    premium_highlights, key=lambda x: x.combined_score, reverse=True
                ...
                premium_highlights_path = os.path.join(
                    self.config.output_dir, "premium_highlights.json"
                )
                ...
                logger.info(
                    f"✅ Premium scoring: {len(final_highlights)} highlights "
    ...
    async def phase2_video_editing(
        self, video_path: str, highlights: List[Dict[str, Any]], subtitles_dir: str
    ) -> Optional[str]:
        ...
        try:
            ...
            output_filename = (
                f"{self.config.project_name}_montage.{self.config.export_format}"
            )
    ...
    async def phase3_quality_control(
        self, video_path: str, highlights: List[Dict[str, Any]], subtitles_dir: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            qc_report_path = os.path.join(
                self.config.output_dir, "reports", f"qc_report_{self.project_id}.json"
            )
            ...
            if self.config.require_human_approval:
                ...
                if qc_report.overall_score >= self.config.min_qc_score:
                    approved = qc_system.human_gate.simulate_human_approval(
                        review_id, approve=True, reviewer="AutoQC"
                    )
                else:
                    approved = False
            ...
            return {"qc_report": None, "approved": False, "qc_score": 0.0}
    ...
    def phase4_finalize_and_report(self, results: Dict[str, Any]) -> PipelineResult:
        ...
        usage_report_path = os.path.join(
            self.config.output_dir, "reports", f"usage_report_{self.project_id}.json"
        )
        ...
        metrics_path = os.path.join(
            self.config.output_dir, "reports", f"metrics_{self.project_id}.json"
        )
        ...
        final_result_path = os.path.join(
            self.config.output_dir, f"pipeline_result_{self.project_id}.json"
        )
...
async def main():
    ...
    parser.add_argument(
        "--max-highlights", type=int, default=5, help="Maximum number of highlights"
    )
    parser.add_argument(
        "--no-premium-ai", action="store_true", help="Disable premium AI scoring"
    )
    parser.add_argument(
        "--require-approval", action="store_true", help="Require human approval"
    )
    ...
    parser.add_argument(
        "--daily-budget", type=float, default=10.0, help="Daily AI budget ($)"
    )
    parser.add_argument(
        "--monthly-budget", type=float, default=200.0, help="Monthly AI budget ($)"
    )
