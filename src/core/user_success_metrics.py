...
class UserSuccessMetrics:
    ...
    def start_session(self, session_id: str, user_id: Optional[str] = None) -> str:
        ...
        session = UserSession(
            session_id=session_id, user_id=user_id, started_at=time.time()
        )
    ...
    def track_upload(self, session_id: str, video_metadata: Dict[str, Any]):
        ...
        if session_id in self.active_sessions:
            ...
            self.db.update(
                "user_sessions",
                {"session_id": session_id},
                {"video_uploaded": True, "metadata": json.dumps(video_metadata)},
            )
    ...
    def track_export(self, session_id: str, segments_kept: int):
        ...
        # Log success pattern
        logger.info(
            f"Success: {session.processing_mode} mode, "
    ...
    def track_abandonment(
        self, session_id: str, stage: str, reason: Optional[str] = None
    ):
    ...
    def track_error(self, session_id: str, error: str):
        ...
        if session_id in self.active_sessions:
            ...
            self.db.update(
                "user_sessions",
                {"session_id": session_id},
                {"error_message": error, "ended_at": datetime.now()},
            )
    ...
    def _generate_insights(
        ...
        if metrics["completion_rate"] < 0.7:
            insights.append(
                f"⚠️ Low completion rate ({metrics['completion_rate']:.1%}). "
                f"Check abandonment stages."
            )
        elif metrics["completion_rate"] > 0.9:
            insights.append(
                f"✅ Excellent completion rate ({metrics['completion_rate']:.1%})!"
            )

        # Retry insight
        if metrics["avg_retries"] > 1.5:
            insights.append(
                f"⚠️ Users averaging {metrics['avg_retries']:.1f} retries. "
        ...
        if metrics["avg_cost"] > 0.5:
            insights.append(
                f"💰 High average cost (${metrics['avg_cost']:.2f}). "
        ...
        if metrics["avg_face_coverage"] < 50:
            insights.append(
                f"📷 Low face coverage ({metrics['avg_face_coverage']:.1%}). "
        ...
        if metrics["median_time"] > 180:
            insights.append(
                f"⏱️ Long processing time ({metrics['median_time']:.0f}s). "
...
async def demo_metrics():
    ...
    # Upload
    success_metrics.track_upload(
        session1, {"duration": 300, "resolution": "1920x1080", "size_mb": 150}
    )
