...
class AlertHandler:
    ...
    def _handle_budget_exceeded(self, job_id: str):
        ...
        self.db.update(
            "video_job",
            {"id": job_id},
            {"status": "failed", "error": "Budget limit exceeded"},
        )
...
class MonitoringServer:
    ...
    def start(self):
        ...
        alert_thread = threading.Thread(
            target=self.alert_handler.start, args=(self.alerts_port,), daemon=True
        )
        alert_thread.start()
    ...
    test_alerts = {
        ...
        "ffmpeg": {
            "alerts": [
                {
                    "status": "firing",
                    "labels": {"alertname": "FFmpegCrashes", "severity": "critical"},
                    "annotations": {
