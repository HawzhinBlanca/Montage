...
try:
    ...
    # NOTE: DaVinci Resolve's Python API is not installable via pip and must be loaded from a file path.
    RESOLVE_SCRIPT_API = "/Library/Application Support/Blackmagic Design/DaVinci Resolve/Developer/Scripting"
    RESOLVE_SCRIPT_LIB = "/Applications/DaVinci Resolve/DaVinci Resolve.app/Contents/Libraries/Fusion/fusionscript.so"

    def _import_resolve_api(path: str):
        """Load DaVinciResolveScript without mutating sys.path."""
        if not Path(path).is_file() and not Path(path).with_suffix(".py").exists():
            raise FileNotFoundError(
                f"DaVinciResolveScript not found at {path}. "
                "Set RESOLVE_SCRIPT_API env variable."
            )

        # Accept directory path or direct .py path
        if Path(path).is_dir():
            script_path = Path(path) / "DaVinciResolveScript.py"
        else:
            script_path = Path(path)

        spec = importlib.util.spec_from_file_location("DaVinciResolveScript", str(script_path))
        module = importlib.util.module_from_spec(spec)  # type: ignore[arg-type]
        assert spec and spec.loader, "Failed to load DaVinciResolveScript module spec"
        spec.loader.exec_module(module)  # type: ignore[assignment]
        return module

    # Canonical import without sys.path mutation
    try:
        dvr_script = _import_resolve_api(RESOLVE_SCRIPT_API)
        RESOLVE_AVAILABLE = True
        print("✅ DaVinci Resolve API available")
    except FileNotFoundError:
        RESOLVE_AVAILABLE = False
        print("⚠️  DaVinci Resolve API not found at", RESOLVE_SCRIPT_API)
