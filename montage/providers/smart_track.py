...
try:
    ...
    # Fallback if cleanup_manager doesn't exist
...
class SmartTrack:
    ...
    def analyze_video(self, video_path: str) -> Dict[str, Any]:
        """Synchronous wrapper around the async process method"""
        import asyncio
        try:
            loop = asyncio.get_event_loop()
            return loop.run_until_complete(self.process(video_path))
        except RuntimeError:
            # No event loop running, create a new one
            return asyncio.run(self.process(video_path))
