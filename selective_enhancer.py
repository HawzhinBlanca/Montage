...
class SelectiveEnhancer:
    ...
    async def enhance_highlights_only(self, 
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                ...
                for j, orig_seg in enumerate(enhanced_result['segments']):
                    if (orig_seg['start_time'] == segment['start_time'] and
                        orig_seg['end_time'] == segment['end_time']):
                ...
                logger.info(f"Enhanced segment {i+1}: ${enhanced_segment.get('enhancement_cost', 0):.2f}")
    ...
    async def process_complex_sections(self,
                                     video_path: str,
                                     complex_sections: List[Tuple[float, float]],
        ...
        for section_start, section_end in complex_sections:
            ...
            try:
                ...
                logger.error(f"Failed to process section {section_start}-{section_end}: {e}")
                continue
        
        return result
    
    async def _enhance_segment(self, 
        ...
        if not segment.get('transcript') and remaining_budget > self.costs['whisper'] * (duration / 60):
            try:
        ...
        if (enhanced.get('transcript') and 
            len(enhanced['transcript'].split()) > 100 and
            remaining_budget - cost > self.costs['gpt_analysis']):
            ...
            try:
                ...
                enhanced['score'] = (enhanced.get('score', 0.5) + gpt_result['score']) / 2
        ...
        if (segment.get('speaker_changes', 0) > 2 and 
            remaining_budget - cost > self.costs['diarization'] * (duration / 60)):
    ...
    async def _process_multi_speaker_section(self,
                                           video_path: str,
                                           start: float,
        ...
        # Extract and diarize
        diarization = await self.diarizer.diarize_section(
    ...
    async def _process_technical_section(self,
                                       video_path: str,
                                       start: float,
                                       end: float) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'metadata': {
                'type': 'technical',
                'concepts_found': sum(len(s.get('technical_concepts', [])) for s in segments)
            }
        }
    
    async def _transcribe_segment(self, 
                                video_path: str,
                                start: float,
                                end: float) -> Dict[str, Any]:
        """
...
"""
    ...
    def _extract_dialogue_highlights(self,
    ...
    def _get_transcript_chunk(self, 
    ...
    async def _analyze_technical_content(self, text: str) -> Dict[str, Any]:
        ...
        technical_keywords = ['algorithm', 'function', 'method', 'system', 'process',
                            'data', 'model', 'architecture', 'design', 'implementation']
    ...
    async def _process_general_complex(self,
                                     video_path: str,
                                     start: float,
                                     end: float) -> Dict[str, Any]:
        """
...
class SpeakerDiarizer:
    ...
    async def diarize_section(self, video_path: str, start: float, end: float) -> Dict[str, Any]:
        await asyncio.sleep(0.1)  # Simulate processing
        
        return {
            'speakers': ['Speaker_A', 'Speaker_B'],
            'segments': [
                {'speaker': 'Speaker_A', 'start': start, 'end': start + (end-start)/3},
                {'speaker': 'Speaker_B', 'start': start + (end-start)/3, 'end': start + 2*(end-start)/3},
                {'speaker': 'Speaker_A', 'start': start + 2*(end-start)/3, 'end': end}
            ]
        }
